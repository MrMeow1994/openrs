...
public class AreaDefinitionTo317Dumper {
    ...
    public static void main(String[] args) throws IOException {
        ...
        try(Cache cache = new Cache(FileStore.open(Constants.SAILING_CACHE_PATH))) {
            AreaTypeList list = new AreaTypeList();
            list.initialize(cache);

            DataOutputStream dat = new DataOutputStream(new FileOutputStream(new File(dir, "areas.dat")));
            DataOutputStream idx = new DataOutputStream(new FileOutputStream(new File(dir, "areas.idx")));
