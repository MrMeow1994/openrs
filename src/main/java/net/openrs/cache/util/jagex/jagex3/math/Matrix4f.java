...
public final class Matrix4f {
   ...
   public Matrix4f(ByteBuffer arg0, boolean arg1) {
      this.deserialise(arg0, arg1);
   }

   void deserialise(ByteBuffer packet, boolean compact) {
      if (compact) {
         Matrix34f matrix = new Matrix34f();
         matrix.rotate_x(Trigonometry14Bits.to_radian(ByteBufferUtils.get_short(packet)));
         matrix.rotate_y(Trigonometry14Bits.to_radian(ByteBufferUtils.get_short(packet)));
         matrix.rotate_z(Trigonometry14Bits.to_radian(ByteBufferUtils.get_short(packet)));
         matrix.translate((float)ByteBufferUtils.get_short(packet), (float)ByteBufferUtils.get_short(packet), (float)ByteBufferUtils.get_short(packet));
         this.set(matrix);
      } else {
         for(int i = 0; i < 16; ++i) {
            this.values[i] = ByteBufferUtils.get_float(packet);
         }
      }
   ...
   float[] get_euler_angles_yxz() {
      ...
      if ((double)this.values[2] < 0.999D && (double)this.values[2] > -0.999D) {
         euler_angles[1] = (float)(-Math.asin(this.values[2]));
         double cos = Math.cos(euler_angles[1]);
         ...
         euler_angles[0] = 0.0F;
         euler_angles[1] = (float)Math.atan2(this.values[2], 0.0D);
         euler_angles[2] = (float)Math.atan2(-this.values[9], this.values[5]);
      }
   ...
   public float[] get_euler_angles_yxz_inverse() {
      float[] euler_angles = new float[]{(float)(-Math.asin(this.values[6])), 0.0F, 0.0F};
      double cos = Math.cos(euler_angles[0]);
      if (Math.abs(cos) > 0.005D) {
         double m02 = this.values[2];
         double m22 = this.values[10];
         double m10 = this.values[4];
         double m11 = this.values[5];
         euler_angles[1] = (float)Math.atan2(m02, m22);
         euler_angles[2] = (float)Math.atan2(m10, m11);
      } else {
         double m01 = this.values[1];
         double m00 = this.values[0];
